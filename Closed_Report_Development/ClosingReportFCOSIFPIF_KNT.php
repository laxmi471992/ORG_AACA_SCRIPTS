<?php
/** ----------------------------------------------------------------------------*
 * Filename: ClosingReportFCOSIFPIF_KNT.php
 * @author      KEANT Technologies
 * @description
 * - Generates FCO SIF PIF reports as XLSX files for each client code and path.
 * - Pulls closing data for the prior month from HSFLCLNTWF and formats it with
 *   standardized headers and styling.
 * - Writes files to per-client directories and transfers them via SFTP based
 *   on company status (active, pending, inactive, terminated).
 * - Logs scheduler status, file size, and notification outcomes, and returns
 *   a summary status array for the run.
 * ----------------------------------------------------------------------------
 * CHANGELOG:
 * ----------------------------------------------------------------------------
...
// Database and SFTP connection helpers.
...
{
        // Track company status buckets for notification.
$Active=false; $Pending=false; $Inactive=false; $Terminated=false;
    ...
    if($userType==3){
        ...
        foreach ($client_codes as $key => $client_code) {
                        // Ensure output directory exists for this client path.
            ...
            if($queryResult['numRows']>0)
            {
                ...
                if($return_header_style_Sheetname)
                {
                 ...
                 // Status 1/3: normal route with SFTP copy.
          ...
          if (!file_exists('/var/www/html/bi/dist/'.$paths[$j].'/'.$filename))  
           {
            ...
            {
                $status = 1;
            // Ensure target directory and send file over SFTP.
        ...
        $FileSizeKB = round($fileSize/1024,2). "KB";
         ...
         if($cmpstatus==4){
            ...
            {
                // Track no-data results for status-specific notifications.
...
if($Active==True){
    // Email notification for active firms with no data.
       ...
       if($Pending==True){
        ...
        // Email notification for pending firms with no data.
       ...
       if($Inactive==True){
        //print_r($Pending_data);
        // Email notification for inactive firms with no data.
        $ind=implode(',', $Inactive_data);
                ...
                include('PHPMailer/mailcode.php');
                   ...
                   // Email notification for terminated firms with no data.
    ...
    // Final success and no-data messaging.
...
{
    // Create a timestamped XLSX filename.
    $d = new DateTime();
...
{ 
    global $pdo;
    // Read company status from CC_REGSTR.
...
{
    global $pdo;
    // Query prior-month closing records for specific closing codes.
...
{
    // Define report headers, styles, and sheet name.
...
function sendsuccessMsg_ClosingReportfcosifpif_1($dataPresents,$userType,$report_start_time,$reportName,$run_by,$connection,$mailNotification,$cmpstatus){
    ...
    if($dataPresents){
        // SFTP delivery and optional email notifications for generated files.
        $msg = 'Report generated successfully';
        $status=1;
        $sftpStatus=0;
        $recipients=array();
        foreach ($dataPresents as $value) {
           ...
           // ssh2_scp_send($connection,'/var/www/html/bi/dist/'.$paths.'/'.$filename, '../../var/www/html/MAKO/downloadfile/AACA/TRM'.'/'.$filename, 0644);
            ...
            if($mailNotification == 1) {
                // Notify recipients if email is enabled.
                $split = explode("/", $paths);
...
{
    // Log no-data outcomes for each client code.
    $msg = 'No Data available';
    $status_msg = 'failed';
    $sftpStatus = 0;
    $status=0;
